...
//!
//! The cert/key pair is persisted under `$XDG_DATA_HOME/lightview/tls/` so
//! that trust decisions survive restarts. It is regenerated only when missing, close to
...
/// 3 = also X.509 nameConstraints, without which a CA:TRUE certificate the
///     user has enabled under iOS Certificate Trust Settings is trusted for
///     **any** host that key signs for — see `generate`.
const CERT_SCHEMA: u32 = 3;
...
/// close to expiry.
fn load_existing(dir: &Path, sans: &[String]) -> Option<TlsMaterial> {
...
fn generate(sans: &[String]) -> Result<(TlsMaterial, i64), String> {
    ...
    // **And name constraints, without which the trade above has no bottom.**
    //
    // Once a CA:TRUE certificate is enabled under iOS Certificate Trust
    // Settings, the phone trusts any server certificate that key signs — for a
    // bank as readily as for the gallery — and the key is a file on a home NAS
    // that any backup, snapshot or rsync of the data directory carries off.
    // Permitted subtrees are exactly the SANs the certificate already names;
    // everything else is excluded. Apple and Chromium both enforce this, the
    // iOS install flow is unchanged, and the blast radius stops being "the
    // internet".
    //
    // RFC 5280: a name form with no permitted entry is *unconstrained*, so both
    // DNS and IP must have at least one entry. Loopback is always a SAN, so the
    // IP list is never empty. The rfc822 exclusion closes the one remaining
    // form a serverAuth certificate could otherwise assert.
    let mut permitted = Vec::new();
    for san in sans {
        match san.parse::<IpAddr>() {
            Ok(IpAddr::V4(ip)) => permitted.push(rcgen::GeneralSubtree::IpAddress(
                rcgen::CidrSubnet::V4(ip.octets(), [255, 255, 255, 255]),
            )),
            Ok(IpAddr::V6(ip)) => permitted.push(rcgen::GeneralSubtree::IpAddress(
                rcgen::CidrSubnet::V6(ip.octets(), [0xff; 16]),
            )),
            Err(_) => permitted.push(rcgen::GeneralSubtree::DnsName(san.clone())),
        }
    }
    params.name_constraints = Some(rcgen::NameConstraints {
        permitted_subtrees: permitted,
        excluded_subtrees: vec![rcgen::GeneralSubtree::Rfc822Name(String::new())],
    });
...
pub fn load_or_generate(dirs: &Dirs, extra_sans: &[String]) -> Result<TlsMaterial, String> {
...
pub fn persisted_cert_pem(dirs: &Dirs) -> Option<Vec<u8>> {
    fs::read(tls_dir(dirs).join(CERT_FILE)).ok()
}
...
mod tests {
    ...
    /// The generated certificate must carry X.509 nameConstraints.
    ///
    /// Checked by looking for the extension's OID in the DER rather than by
    /// pulling in an x509 parser: `2.5.29.30` encodes as `06 03 55 1D 1E`. The
    /// failure it guards is severe and silent — a CA:TRUE certificate the user
    /// has enabled on their phone, with no limit on what it may vouch for.
    #[test]
    fn the_certificate_is_name_constrained() {
        let sans = san_strings(v4(192, 168, 1, 10), &[]);
        let (material, _) = generate(&sans).expect("generate");

        let pem = String::from_utf8(material.cert_pem.clone()).unwrap();
        let der = pem
            .lines()
        ...
        const NAME_CONSTRAINTS_OID: &[u8] = &[0x06, 0x03, 0x55, 0x1d, 0x1e];
        assert!(
            der.windows(NAME_CONSTRAINTS_OID.len())
                .any(|w| w == NAME_CONSTRAINTS_OID),
            "the certificate has no nameConstraints extension"
        );

        // And basicConstraints CA:TRUE is still there, since the constraint
        // only matters because the certificate is installable as an anchor.
        const BASIC_CONSTRAINTS_OID: &[u8] = &[0x06, 0x03, 0x55, 0x1d, 0x13];
        assert!(
            der.windows(BASIC_CONSTRAINTS_OID.len())
                .any(|w| w == BASIC_CONSTRAINTS_OID),
            "the certificate is no longer a CA, so it cannot be trusted on iOS"
        );
    }

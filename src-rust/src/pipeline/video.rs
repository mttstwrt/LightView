...
pub fn sample_timestamps(duration: f64, count: u32) -> Vec<f64> {
    // `!duration.is_finite()` rather than a comparison: ffprobe reports NaN
    // for a stream with no usable duration, and every ordering comparison
    // against NaN is false — so `duration <= 0.0` would let it through.

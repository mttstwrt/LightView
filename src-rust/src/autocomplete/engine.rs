//! Fuzzy tag suggestion from an in-memory copy of the tag vocabulary.
//!
//! The whole vocabulary is held in RAM — roughly 300 KB at 5,000 unique tags —
//! so there is no cache-eviction policy to reason about and a query is a linear
//! scan with no I/O.
//!
//! It is fed by one `SELECT namespace, tag, COUNT(*) FROM tag_index GROUP BY 1, 2`
//! at each refresh. There is no `tag_counts` table: that one was keyed
//! `(namespace, tag)` rather than by path, so it sat outside the path-keyed
//! sweep this design says has no exceptions, needed two maintenance paths of
//! its own, and was rebuilt per apply batch at a cost that scaled with the
//! library rather than the batch. This engine already holds every tag with a
//! count, at the moments the caller already refreshes it, so the table was a
//! second copy of what is in this struct.
//!
...
/// One row of the vocabulary: a tag, its namespace, and how many files carry

...
//! inclusive Unix seconds, `Width`/`Height` are pixels, and `FileSize` is
//! bytes.
//!
//! `GeoBbox` is gone with the map view that was its only caller. `has:geo`
//! stays, because "does this have coordinates at all" is a question the grid
//! asks and a bounding box is not.
...
pub enum TagNamespace {
    User,
    /// Set membership. A sibling of `User` rather than a plugin bucket,
    /// because a set is user-owned and must survive a plugin re-run.
    Set,
    Plugin(String),
...
impl TagNamespace {
    ...
    pub fn to_db_namespace(&self) -> Option<String> {
        match self {
            ...
            TagNamespace::Set => Some("set".to_string()),

...
///   NOT plugin.wd::indoor                       → boolean NOT
...
///   (user::a OR user::b) AND NOT set::burst     → grouped expression
...
///
/// Three context-sensitive characters, and each earns it:
///
/// - `"` opens and closes a quoted run, inside which whitespace and
///   parentheses are literal. Without it **a tag containing a space cannot be
///   named by any query** — it fails outright, and autocomplete offers such
///   tags, so clicking a suggestion returned a 500. The quotes are delimiters
///   and are not part of the token, and a quote may open mid-token so
///   `user::"my tag"` works.
/// - `(` groups only when it *starts* a token, which is what keeps
///   `hatsune_miku_(vocaloid)` intact.
/// - `)` closes a group unless it is balancing an in-tag `(`.
///
/// An unterminated quote runs to the end of the input rather than erroring:
/// the filter bar parses on every keystroke, so a half-typed quote must not
/// flash an error.
...
mod tests {
    ...
    #[test]
    fn set_namespace_parses_like_user() {
        match parse_filter("set::kellys-comic").unwrap() {
            FilterExpr::Tag { namespace, value } => {
                assert_eq!(namespace, TagNamespace::Set);
                assert_eq!(value, "kellys-comic");
            }
            other => panic!("Expected Tag, got {other:?}"),
        }
        assert!(matches!(
            parse_filter("has::set").unwrap(),
            FilterExpr::HasNamespace {
                namespace: TagNamespace::Set
            }
        ));
        assert!(matches!(
            parse_filter("set").unwrap(),
            FilterExpr::HasNamespace {
                namespace: TagNamespace::Set
            }
        ));
    ...
    fn a_tag_containing_a_space_can_be_named() {
        ...
        match parse_filter("\"two words\"").unwrap() {
            FilterExpr::Tag { namespace, value } => {
    ...
    #[test]
    fn a_quote_may_open_mid_token() {
        match parse_filter("user::\"my tag\"").unwrap() {
            FilterExpr::Tag { namespace, value } => {
                assert_eq!(namespace, TagNamespace::User);
                assert_eq!(value, "my tag");
            }
            other => panic!("Expected Tag, got {other:?}"),
        }
    }
    ...
    #[test]
    fn quoting_suspends_the_grouping_and_operator_rules() {
        // Inside quotes a parenthesis and the word AND are ordinary text.
        match parse_filter("\"a (b) AND c\"").unwrap() {
            FilterExpr::Tag { value, .. } => assert_eq!(value, "a (b) AND c"),
            other => panic!("Expected Tag, got {other:?}"),
        }
    }

    #[test]
    fn an_unterminated_quote_runs_to_the_end_rather_than_failing() {
        // The filter bar parses on every keystroke; a half-typed quote must
        // not flash an error.
        match parse_filter("\"half typed").unwrap() {
            FilterExpr::Tag { value, .. } => assert_eq!(value, "half typed"),
            other => panic!("Expected Tag, got {other:?}"),
        }
    }

    #[test]
    fn quoted_terms_compose_with_operators() {
        match parse_filter("\"two words\" AND user::solo").unwrap() {
            FilterExpr::And { left, right } => {
                assert!(matches!(left.as_ref(), FilterExpr::Tag { .. }));
                assert!(matches!(right.as_ref(), FilterExpr::Tag { .. }));
            }
    ...
    #[test]
    fn test_not() {
        let expr = parse_filter("NOT set::burst-3").unwrap();
    ...
    #[test]
    fn test_complex() {
        let expr = parse_filter("(user::vacation OR user::travel) AND NOT set::burst-3").unwrap();
        assert!(matches!(expr, FilterExpr::And { .. }));
    }

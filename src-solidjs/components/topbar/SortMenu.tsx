...
export function SortMenu(props: { dropUp?: boolean }) {
  ...
  const reSort = async (
    ...
    try {
      ...
      await refresh({
        sort: field,
        order,
        // Custom is a total order arranged by hand: nothing to break a tie.
        sub_sort: field === "custom" ? null : subField,
        sub_order: subOrder,
        group_by: groupBy(),
      });
    } catch (e) {
  ...
  const handleSelectField = (field: SortField) => {
    if (field === "custom") {
      // No direction to toggle: reversing a hand-made order means nothing.
      if (!isCustom()) reSort("custom", "asc", subSortField(), subSortOrder());
  ...
  const isCustom = () => sortField() === "custom";
  ...
  // Sub-sort options: everything except the current primary field, and never
  // Custom, which has no ties to break and breaks none of anyone else's.
  const subOptions = () =>
    SORT_OPTIONS.filter((o) => o.field !== sortField() && o.field !== "custom");
